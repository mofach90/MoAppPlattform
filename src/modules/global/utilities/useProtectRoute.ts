...
const useProtectRoute: UseProtectRoute = () => {
  ...
  const [isMainRouteProtected, setMainIsRouteProtected] = useState<boolean>(false);
  const [isDemoRouteProtected, setDemoIsRouteProtected] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    if (!isAuthCheckloading) {
      const currentAuthFlow = getCurrentAuthFlow(authenticationForm, authStates);
      console.log('currentAuthFlow:', currentAuthFlow);
      
      const isAuthenticated = currentAuthFlow && currentAuthFlow.isAuthenticated;
      console.log('isAuthenticated:', isAuthenticated);
      
      if (isAuthenticated && authenticationForm === 'Firebase based authentication using Email and Password or Anonymously') {
        console.log('Setting main route protected');
        setMainIsRouteProtected(true);
      } else if (isAuthenticated && authenticationForm !== 'Firebase based authentication using Email and Password or Anonymously') {
  ...
  return { isMainRouteProtected, isDemoRouteProtected, currentLoginPath,isLoading };
};
